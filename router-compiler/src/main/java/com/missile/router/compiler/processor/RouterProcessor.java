package com.missile.router.compiler.processor;

import com.google.auto.service.AutoService;
import com.missile.router.annotation.Module;
import com.missile.router.annotation.Modules;
import com.missile.router.annotation.Router;
import com.missile.router.compiler.utils.Consts;
...
import static com.missile.router.compiler.utils.Consts.ANNOTATION_TYPE_MODULE;
import static com.missile.router.compiler.utils.Consts.ANNOTATION_TYPE_MODULES;
import static com.missile.router.compiler.utils.Consts.ANNOTATION_TYPE_ROUTER;

@AutoService(Processor.class)
@SupportedSourceVersion(SourceVersion.RELEASE_7)
@SupportedAnnotationTypes({ANNOTATION_TYPE_ROUTER, ANNOTATION_TYPE_MODULE, ANNOTATION_TYPE_MODULES})
public class RouterProcessor extends AbstractProcessor {
    ...
    public boolean process(Set<? extends TypeElement> annotations, RoundEnvironment roundEnv) {
        ...
        // module
        String moduleName = Consts.MODULE_NAME;
        ...
        if (modulesList != null && modulesList.size() > 0) {
            Modules annotation = modulesList.iterator().next().getAnnotation(Modules.class);
    ...
    private void generateModulesRouterInit(String[] moduleNames) {
        ...
        TypeSpec routerInit = TypeSpec.classBuilder("RouterInit")
                ...
                .build();
        try {
            JavaFile.builder("com.missile.router", routerInit)
                    .build()
                    .writeTo(filer);
        } catch (Exception e) {
            e.printStackTrace();
    ...
    private boolean handleRouter(String genClassName, RoundEnvironment roundEnv) {
        ...
        MethodSpec.Builder mapMethod = MethodSpec.methodBuilder("map")
                ...
                .addStatement("com.missile.router.ExtraTypes extraTypes")
                .addCode("\n");
        ...
        for (Element element : elements) {
            ...
            mapMethod.addStatement("extraTypes = new com.missile.router.ExtraTypes()");
            ...
            for (String format : router.value()) {
                ...
                if (element.getKind() == ElementKind.CLASS) {
                    mapMethod.addStatement("com.missile.router.Routers.map($S, $T.class, null, extraTypes)", format, className);
                } else {
                    mapMethod.addStatement("com.missile.router.Routers.map($S, null, " +
                            "new MethodInvoker() {\n" +
        ...
        TypeSpec routerMapping = TypeSpec.classBuilder(genClassName)
                ...
                .build();
        try {
            JavaFile.builder("com.missile.router", routerMapping)
                    .build()
                    .writeTo(filer);
        } catch (Throwable e) {
            e.printStackTrace();
